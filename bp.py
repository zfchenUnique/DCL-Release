...
class ProgramExecutorContext(nn.Module):
    ...
    def filter_ancestor(self, event_list):
        ...
        frm_weight = torch.tensor(frm_mask_list, dtype= self.event_list[0].dtype, device = self.event_list[0].device)
        ...
        colli_mask = torch.min(colli_mask, objset_weight.unsqueeze(-1))
    ...
    def filter_collision(self, selected, group, concept_groups):
        ...
        if selected is not None:
            colli_mask = torch.min(colli_mask, selected.unsqueeze(-1))
            pdb.set_trace()
...
class DifferentiableReasoning(nn.Module):
    ...
    def forward(self, batch_features, progs_list, fd=None):
        ...
        programs_list = programs_list_oe + programs_list_mc
        buffers_list = buffers_list_oe + buffers_list_mc 
        result_list = result_list_oe + result_list_mc 
